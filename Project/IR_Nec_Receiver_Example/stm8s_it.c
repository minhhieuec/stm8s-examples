...
/**
  ...
  * @retval None
  */
  uint16_t count_ms1=0, count_ms2=0, count_ms3=0;
 int8_t count_data=0;
 uint8_t start_status=0;
 uint32_t bitPattern=0;
 uint8_t count_zero=0;
 uint8_t count_one=0;
 extern void Dec2hex(unsigned long dec);
 extern void ir_cmd_recv_process(uint32_t val);
INTERRUPT_HANDLER(EXTI_PORTB_IRQHandler, 4)
{
  ...
  */
  ///
    if ((GPIO_ReadInputData(GPIOB) & 0x20) == 0x00)///10.0000
  {
      ...
      {
        if((count_ms1>=4)&(count_ms1<=6))//2s->3s
        {
          bitPattern |=(uint32_t)1<<(31-count_data);
          count_ms1=0;
          count_one++;
        }
        else if(count_ms1<4)//nho hon <2s
        {
          count_ms1=0;
          count_zero++;
        }
      ...
      else if((start_status=0)&((count_ms1)>28))
      {
          count_ms1=0;
          start_status=0;
          count_data=0;
      ...
      {
        Dec2hex(bitPattern);
        ir_cmd_recv_process(bitPattern);
        ///
        count_data=0;
        bitPattern=0;
        start_status=0;
